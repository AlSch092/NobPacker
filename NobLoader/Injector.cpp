...
bool injected = false;
uintptr_t g_ShellcodeAddr = 0;
BYTE* g_MappingData = nullptr;

bool ManualMapDll(bool bShouldUnpack, bool bShouldDecrypt, uintptr_t DecryptKey, HANDLE hProc, BYTE* pSrcData, SIZE_T FileSize, bool ClearHeader, bool ClearNonNeededSections, bool AdjustProtections, bool SEHExceptionSupport, DWORD fdwReason, LPVOID lpReserved) {
	...
	pOldFileHeader = &pOldNtHeader->FileHeader; //headers likely need section data/sizes updates for unpacked code
	...
	if (bShouldUnpack)
	{
		bool ok = UnpackSections(pSrcData, FileSize, pTargetBase, pOldOptHeader->SizeOfImage, bShouldDecrypt, DecryptKey);
		if (!ok) 
		{
			printf("No packed sections found or unpack failed\n");
		}
	}
	...
	//for (UINT i = 0; i != pOldFileHeader->NumberOfSections; ++i, ++pSectionHeader) {  // NOT NEEDED! unpacking func takes care of this already
	//	//if (pSectionHeader->SizeOfRawData) {
	//		if (strcmp((const char*)pSectionHeader[i].Name, ".text") != 0 && strcmp((const char*)pSectionHeader[i].Name, ".rdata") != 0)
	//		{
	...
	g_ShellcodeAddr = (uintptr_t)pShellcode;
	g_MappingData = (BYTE*)MappingDataAlloc;
	...
	//in DEBUG build, Shellcode points to a jmp table rather than the actual func!! causing a crash
	...
	while (!hCheck) 
	{
		...
		if (hCheck == (HINSTANCE)0x404040) 
		{
			...
			VirtualFreeEx(hProc, pTargetBase, 0, MEM_RELEASE);
			VirtualFreeEx(hProc, MappingDataAlloc, 0, MEM_RELEASE); //test this after
			VirtualFreeEx(hProc, pShellcode, 0, MEM_RELEASE);
			return false;
		}
